...
title: Java数值与金额精度
description: "深入分析浮点数精度丢失的根本原因(IEEE 754二进制表示);讲解为什么金额计算不能用float/double;对比BigDecimal不同构造方式的差异与内部表示(无标度值与标度);讨论金额处理的两种方案(BigDecimal元 vs Long分);解析equals与compareTo的等值判断差异;提供费率计算、四舍五入等实战建议。"
