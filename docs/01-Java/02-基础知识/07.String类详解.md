...
title: String 类详解
description: "深入剖析String不可变性的实现原理与设计动机(常量池、安全性、线程安全、HashCode缓存);全面对比String、StringBuilder、StringBuffer的性能与适用场景;图解字符串常量池机制、new String()创建对象数量、intern()方法原理;讲解编译期与运行期的长度限制;介绍 JDK 9的Compact String存储优化与StringConcatFactory拼接优化。"
