...
output {
  elasticsearch {
    hosts => ["http://es:9200"]
    index => "damai-logs-%{projectName}-%{+YYYY.MM.dd}"
    user => "elastic"
    password => "elastic"
  }
...
### 应用集成配置
在大麦项目的 `log4j2.xml` 中添加 Socket Appender：

```xml
<Socket name="Logstash" host="127.0.0.1" port="5047" protocol="TCP">
    <ThresholdFilter level="INFO"/>
    <DamaiJsonLayout projectName="${PROJECT_NAME}" env="${ENV}" 
                     charset="${ENCODING}" complete="false" compact="true" 
                     eventEol="true" locationInfo="true" includeStacktrace="true" />
</Socket>
```

**注意：**
- `host="127.0.0.1"`: 应用在宿主机运行时使用
- 如果应用也在 Docker 容器中，需改为 `host="logstash"` 并加入 elk 网络
- `packages="com.damai.log4j"`: 在 Configuration 标签中指定自定义插件的包路径
